<template>
    <div class="w-full flex flex-col h-screen bg-gray-100">
      ...
      <div class="p-3 border-b bg-gray-200 flex items-center">
        <img :src="chat.avatar || require('@/assets/images/user.png')" class="w-10 h-10 rounded-full">
      ...
      <div class="flex-grow overflow-y-auto p-3">
        ...
        <div v-else-if="messages.length === 0" class="text-center text-gray-500">Nessun messaggio</div>
  ...
  export default {
    ...
    methods: {
      async fetchMessages() {
        try {
          const response = await axios.get(`/conversations/${this.chat.id}`);
